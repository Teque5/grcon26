...
def generate_dual_tone(fs, f1, f2, duration):
    ...
    return dual_tone * 0.25


def message_to_ascii_digits(message):
    """
    Encode input message string to ascii ints, concatenate all those int digits
    together and return a list of digit characters
...
def generate_message_wav(message, filename="message.wav", dial_tone_duration=3):
    ...
    digits = message_to_ascii_digits(message)
    for num in digits:
        f1, f2 = tone_map[num]
...
if __name__ == "__main__":
    generate_message_wav("flag{ITU-TRecQ.23}")
